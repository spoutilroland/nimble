...
export function SectionRow({ section, index, total, layouts, isEven = false, onRemove, onMoveUp, onMoveDown, onUpdate, onSave }: SectionRowProps) {
  const { t } = useI18n();
  ...
  return (
    <div className={`flex flex-col py-2 px-[0.8rem] border border-[var(--bo-border)] ${isEven ? 'bg-[rgba(255,255,255,0.04)]' : 'bg-[rgba(99,102,241,0.08)]'}`} data-type={section.type}>
      <div className="flex items-center gap-2">
        ...
        <span className="w-[180px] shrink-0 text-[0.85rem] text-[var(--bo-text)] truncate">{t(`sectionType.${section.type}`)}</span>

        <div className="section-row-btns flex gap-[0.25rem] shrink-0">
